...
void ModeHanding::run()
{
    ...
    // fixed attitude: no XY motion, no yaw rotation
    // const float target_roll_rad      = 0.0f;
    // const float target_pitch_rad     = 0.0f;
    ...
    // state machine
    HandingModeState poshold_state = get_handing_state_U_ms(target_climb_rate_ms);
...
float ModeHanding::mix_controls(float mix_ratio, float first_control, float second_control)
{
...
void ModeHanding::update_brake_angle_from_velocity(float &brake_angle_rad, float velocity_ms)
{
...
void ModeHanding::init_wind_comp_estimate()
{
...
void ModeHanding::update_wind_comp_estimate()
{
    const uint32_t now_ms = AP_HAL::millis();
...
void ModeHanding::get_wind_comp_lean_angles_rad(float &roll_angle_rad, float &pitch_angle_rad)
{
